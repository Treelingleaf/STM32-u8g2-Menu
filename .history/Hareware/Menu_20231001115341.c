...
void Creat_Childv2_Menu(ChildMenu *menuitem, uint8_t Picture_Flag, char *Title)
{
	...
	{
		Box_Flag = 0;
		Menu_To_X_Display(menuitem, Title);
		while (1)
		{
...
*/
void Menu_To_X_Display(ChildMenu *menuitem, char *Title) // 菜单→游戏菜单过渡动画
{
	...
	while (Game_Menu_Display != Game_Menu_Display_trg)
	{
		u8g2_ClearBuffer(&u8g2);
		uint8_t str_x = (128 - (strlen(menuitem->father->name) * 6)) / 2;
		u8g2_DrawStr(&u8g2, str_x, Game_Menu_Display, menuitem->father->name); // Game_Menu_Display+16+62为72，此时“Game”消失
		u8g2_DrawLine(&u8g2, 1, Game_Menu_Display + 3, 128, Game_Menu_Display + 3);
		for (int i = 0; i < 4; i++)
		{
