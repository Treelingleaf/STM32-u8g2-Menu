...
void Show_Menu(Speed_ENUM Speed_choose) // 显示菜单
{
	...
	case 0:
		...
		u8g2_SendBuffer(&u8g2);
		Creat_Childv2_Menu(MainMenu_Config[Picture_Flag].ChildMenu, Picture_Flag, MainMenu_Config[Picture_Flag].MenuName);
		break;
...
void Creat_Childv2_Menu(ChildMenu *menuitem, uint8_t Picture_Flag, char *Title)
{
	...
	{
		Box_Flag = 0;
		Menu_To_X_Display(menuitem, Title, Box_Flag);
		while (1)
		{
...
*/
void Menu_To_X_Display(ChildMenu *menuitem, char *Title, uint8_t Box_Flag) // 菜单→游戏菜单过渡动画
{
	...
	while (Game_Menu_Display != Game_Menu_Display_trg)
	{
		...
		{
			uint8_t str_x = (128 - (strlen(menuitem->father[Box_Flag].name) * 6)) / 2;
			u8g2_DrawStr(&u8g2, str_x, Game_Menu_Display, menuitem->father[Box_Flag].name);
		}
		u8g2_DrawLine(&u8g2, 1, Game_Menu_Display + 3, 128, Game_Menu_Display + 3);
		for (int i = 0; i < 4; i++)
		{
