...
void Creat_Childv2_Menu(ChildMenu *menuitem, char *Title, uint8_t child_Flag)
{
	...
	Box_y = 15 + 12 * Box_Flag;
	Box_w = strlen(menuitem[Box_Flag].name) * 6 + 6;
	...
	if (Game_Menu_Flag == 3) // 确认键
	{
		...
		while (1)
		{
			...
			ui_run(&Box_x, &Box_x_trg, 5);
			ui_run(&Box_y, &Box_y_trg, 5);
			ui_run(&Box_w, &Box_w_trg, 5);
			ui_run(&Box_h, &Box_h_trg, 5);
