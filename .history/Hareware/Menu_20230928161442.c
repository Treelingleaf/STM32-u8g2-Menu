...
/*定义二级子菜单结构体*/
ChildMenu GameMune[4] = {{"game1:FlyBird", NULL}, {"game2:Dinosaur Rex", NULL}, {"game3:Stick Fight", NULL}, {"game4:Tetris", NULL}};

ChildMenu SettingMune[4] = {
	{"setting1:FlyBird", NULL},
	{"setting2:Dinosaur Rex", NULL},
	{"setting3:Stick Fight", NULL},
	{"setting4:Tetris", NULL}};
...
void Show_Menu(Speed_ENUM Speed_choose) // 显示菜单
{
	...
	case 0:
		...
		u8g2_SendBuffer(&u8g2);
		Creat_ChildV2_Menu(MainMenu_Config, Picture_Flag, MainMenu_Config[Picture_Flag].MenuName);
		break;
	...
	if (Game_Menu_Flag == 3) // 确认键
	{
		...
		while (1)
		{
			...
			case 0:
				Box_x_trg = 1;
				Box_y_trg = 14;
				Box_w_trg = strlen(MainMenu[Picture_Flag].ChildMenu[Box_Flag].name) * 6 + 6;
				Box_h_trg = 13;
...
void Creat_Childv2_Menu(ChildMenu *menuitem, uint8_t Picture_Flag, char *Title)
{
	...
	if (Game_Menu_Flag == 3) // 确认键
	{
		...
		while (1)
		{
			...
			// else
			// 	//子菜单创建二级子菜单
			// 	u8g2_DrawStr(&u8g2, 52, 10, child_father_name);
			u8g2_DrawStr(&u8g2, 52, 10, Title);
