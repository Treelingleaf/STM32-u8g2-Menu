...
void Show_Menu(Speed_ENUM Speed_choose) // 显示菜单
{
	...
	case 0:
		...
		str_x = (128 - (strlen(&words[0][0]) * 6)) / 2;
		u8g2_DrawStr(&u8g2, str_x, 10, &words[0][0]);
		u8g2_SendBuffer(&u8g2);
		Creat_Childv2_Menu(MainMenu_Config[Picture_Flag].ChildMenu, Picture_Flag, MainMenu_Config[Picture_Flag].MenuName, 0);
		break;

	case 1:
		// u8g2_DrawStr(&u8g2, 52, 10, "MENU:");
		str_x = (128 - (strlen(&words[1][0]) * 6)) / 2;
		u8g2_DrawStr(&u8g2, str_x, 10, &words[1][0]);
	...
	case 2:
		...
		str_x = (128 - (strlen(&words[2][0]) * 6)) / 2;
		u8g2_DrawStr(&u8g2, str_x, 10, &words[2][0]);
		u8g2_SendBuffer(&u8g2);
		Creat_Childv2_Menu(MainMenu_Config[Picture_Flag].ChildMenu, Picture_Flag, MainMenu_Config[Picture_Flag].MenuName, 0);
		break;
	...
	case 3:
		...
		str_x = (128 - (strlen(&words[0][0]) * 6)) / 2;
		u8g2_DrawStr(&u8g2, str_x, 10, &words[3][0]);
		u8g2_SendBuffer(&u8g2);
		Creat_Childv2_Menu(MainMenu_Config[Picture_Flag].ChildMenu, Picture_Flag, MainMenu_Config[Picture_Flag].MenuName, 0);
		break;
	}
...
void Creat_Childv2_Menu(ChildMenu *menuitem, uint8_t Picture_Flag, char *Title, uint8_t child_Flag)
{
	...
	{
		Box_Flag = 0;
		Menu_To_X_Display(menuitem, Title, child_Flag);
		while (1)
		{
			...
			{
				u8g2_ClearBuffer(&u8g2);
				X_To_Menu_Display(menuitem, Title);
				Game_Menu_Flag = 0;
				break;
			}
			...
			case 0:
				...
				if (menuitem[Box_Flag].child != NULL)
					Creat_Childv2_Menu(menuitem[Box_Flag].child, Picture_Flag, child_father_name, Box_Flag);
				break;
			case 1:
				Box_x_trg = 1;
				Box_y_trg = 27;
...
*/
void Menu_To_X_Display(ChildMenu *menuitem, char *Title, uint8_t child_Flag) // 菜单→游戏菜单过渡动画
{
	...
	if (menuitem[0].opt == MAIN_TO_CHILD)
	{
		while (Game_Menu_Display != Game_Menu_Display_trg)
		{
			...
			// TODO:根据标题长度进行居中处理
			uint8_t str_x = (128 - (strlen(Title) * 6)) / 2;
			u8g2_DrawStr(&u8g2, str_x, Game_Menu_Display, Title);				// Y要变为0或72“MENU”才会完全消失
	...
	else if (menuitem[0].opt == CHILD_TO_CHILD)
	{
		while (Game_Menu_Display != Game_Menu_Display_trg)
		{
			u8g2_ClearBuffer(&u8g2);
			uint8_t str_x = (128 - (strlen(menuitem->father->name) * 6)) / 2;
			u8g2_DrawStr(&u8g2, str_x, Game_Menu_Display, menuitem->father->name);
	...
	while (Game_Menu_Display != Game_Menu_Display_trg)
	{
		...
		{
			uint8_t str_x = (128 - (strlen(Title) * 6)) / 2;
			u8g2_DrawStr(&u8g2, str_x, Game_Menu_Display, Title);
		}
		else if (menuitem[0].opt == CHILD_TO_CHILD)
		{
			uint8_t str_x = (128 - (strlen(menuitem->father[child_Flag].name) * 6)) / 2;
			// u8g2_DrawStr(&u8g2, str_x, Game_Menu_Display, menuitem->name);
		}
		u8g2_DrawLine(&u8g2, 1, Game_Menu_Display + 3, 128, Game_Menu_Display + 3);
		for (int i = 0; i < 4; i++)
		{
...
@menuitem：指向MenuItem结构体数组的指针
*/
void X_To_Menu_Display(ChildMenu *menuitem, char *Title)
{
	char Game_Menu_Display = 10;
	char Game_Menu_Display_trg = 74;
