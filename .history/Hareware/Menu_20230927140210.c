...
void Show_Menu(Speed_ENUM Speed_choose) // 显示菜单
{
	...
	else if ((KeyNum == 2) && (display < 48))
	{
		...
		;
	// TODO:显示菜单名
	switch (Picture_Flag) // 根据当前图片标志位确定菜单名
	{
	case 0:
		u8g2_DrawStr(&u8g2, 52, 10, "MENU:");
	...
	case 3:
		u8g2_DrawStr(&u8g2, 52, 10, "MENU:");
		u8g2_DrawStr(&u8g2, 82, 10, &words[3][0]);
		u8g2_SendBuffer(&u8g2);
		break;
	}
}
...
void Game_Menu(void)
{
	...
	if (Game_Menu_Flag == 3) // 确认键
	{
		...
		while (1)
		{
			...
			u8g2_DrawStr(&u8g2, 52, 10, "GAME");  // 菜单名
			u8g2_DrawLine(&u8g2, 1, 13, 128, 13); // 分割线
