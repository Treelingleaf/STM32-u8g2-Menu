...
void Creat_Childv2_Menu(ChildMenu *menuitem, char *Title, uint8_t child_Flag)
{
	...
	static char Box_y_trg_temp;
	static char Box_w_trg_temp;
	...
	if (Game_Menu_Flag == 3) // 确认键
	{
		...
		Box_y_trg_temp = 15 + 12 * Box_Flag;
		Box_w_trg_temp = strlen(menuitem[Box_Flag].name) * 6 + 6;
		...
		while (1)
		{
			...
			ui_run(&Box_x, &Box_x_trg, 4);
			ui_run(&Box_y, &Box_y_trg, 4);
			ui_run(&Box_w, &Box_w_trg, 4);
			ui_run(&Box_h, &Box_h_trg, 4);
			...
			if (Game_Menu_Flag == 4)
			{
				...
				if (menuitem->opt == CHILD_TO_CHILD)
				{
					...
					Box_y_trg = Box_y_trg_temp;
					Box_w_trg = Box_w_trg_temp;
				}
				Game_Menu_Flag = 0;
				break;
			}
