...
ChildMenu GameChild4[4] = {
	{"this is set1", NULL, NULL, CHILD_TO_CHILD},
	{"this is set2", NULL, NULL, CHILD_TO_CHILD},
	{"this is set3", NULL, NULL, CHILD_TO_CHILD},
	{"this is set4", NULL, NULL, CHILD_TO_CHILD}};
...
void Show_Menu(Speed_ENUM Speed_choose) // 显示菜单
{
	...
	case 0:
		...
		u8g2_SendBuffer(&u8g2);
		Creat_Childv2_Menu(MainMenu_Config->ChildMenu, NULL, Picture_Flag, MainMenu_Config[Picture_Flag].MenuName);
		break;
...
void Creat_Childv2_Menu(ChildMenu *menuitem, ChildMenu *childTemp, uint8_t Picture_Flag, char *Title)
{
	...
	if (Game_Menu_Flag == 3) // 确认键
	{
		...
		Box_Flag = 0;
		Menu_To_X_Display(menuitem, childTemp);
		while (1)
		{
			...
			{
				u8g2_ClearBuffer(&u8g2);
				X_To_Menu_Display(menuitem, childTemp);
				Game_Menu_Flag = 0;
				break;
			}
			...
			case 0:
				...
				if (menuitem[Box_Flag].child != NULL)
					Creat_Childv2_Menu(menuitem[Box_Flag].child, childTemp, Picture_Flag, child_father_name);
				break;
			case 1:
				Box_x_trg = 1;
				Box_y_trg = 27;
...
*/
void Menu_To_X_Display(ChildMenu *menuitem, ChildMenu *childTemp) // 菜单→游戏菜单过渡动画
{
	...
	else if (menuitem[0].opt == CHILD_TO_CHILD)
	{
		while (Game_Menu_Display != Game_Menu_Display_trg)
		{
			...
			for (int i = 0; i < 4; i++)
			{
				u8g2_DrawStr(&u8g2, 3, Game_Menu_Display + 14 + 12 * i, childTemp[i].name);
			}
...
@menuitem：指向MenuItem结构体数组的指针
*/
void X_To_Menu_Display(ChildMenu *menuitem, ChildMenu *childTemp)
{
	char Game_Menu_Display = 10;
	char Game_Menu_Display_trg = 74;
