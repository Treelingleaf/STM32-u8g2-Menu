...
void Creat_Childv2_Menu(ChildMenu *menuitem, uint8_t Picture_Flag, char *Title, uint8_t child_Flag)
{
	...
	{
		Box_Flag = 0;
		Menu_To_X_Display(menuitem, child_Flag, Picture_Flag);
		while (1)
		{
			...
			{
				u8g2_ClearBuffer(&u8g2);
				X_To_Menu_Display(menuitem, child_Flag, Picture_Flag);
				Game_Menu_Flag = 0;
				break;
			}
...
*/
void Menu_To_X_Display(ChildMenu *menuitem, uint8_t child_Flag, uint8_t Picture_Flag) // 菜单→游戏菜单过渡动画
{
...
@menuitem：指向MenuItem结构体数组的指针
*/
void X_To_Menu_Display(ChildMenu *menuitem, uint8_t child_Flag, uint8_t Picture_Flag)
{
	char Game_Menu_Display = 10;
	char Game_Menu_Display_trg = 74;
	uint8_t str_x;
